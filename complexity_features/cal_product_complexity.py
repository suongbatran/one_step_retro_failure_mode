...
import argparse
from complexity_features.product_complexity import *
...
def process_row(row):
    ...
    row['npscore'] = -9999
    row['sps'] = -9999
    row['boettcher'] = -9999 # this is just a placeholder to indicate failing to calculate the Boettcher score. Only 4 cases in USPTO (after preprocessing)
    try:
    ...
    try:
        row['sascore'] = get_SAScore(product_mol)
    except Exception:
        pass 
    try:
        row['npscore'] = get_NPScore(product_mol)
    except Exception:
        pass 
    try:
        row['sps'] = get_SPScore(product_mol)
    except Exception:
        pass
    try:
        row['boettcher'] = get_BoettcherScore(product_smiles)
    except Exception:
        pass 
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Get product complexity features from reaction SMILES')
